...
# Близки ли к нулю элементы строки
def zeros(row, n):
    for i in range(n):
...
# Модифицированный метод решения системы с совместной матрицей
...
# Модифицированный метод решения системы с вырожденной матрицей
def system_solution_3(matrix_l, matrix_u, matrix_p, matrix_q, b, rank, n):
    operations_number = 0
    cU = np.copy(matrix_u)
...
def SLAE_solve(matrix, _b, n):
    ...
    if np.linalg.matrix_rank(Ab) == rank:
        if rank == n:
            ...
            x, count = system_solution_3(matrix_l, matrix_u, matrix_p, matrix_q, _b, rank, n)
    return x, count
    ...
    while norm > accuracy:
        ...
        if np.linalg.matrix_rank(Ab) == rank:
            if rank == n:
                ...
                apr_x, count = system_solution_3(matrix_l, matrix_u, matrix_p, matrix_q, _b, rank, n)
        operation_num += count
        iter_num += 1
        norm = np.linalg.norm(apr_x, np.inf)
        x_cur = apr_x + x_cur
...
# Метод Ньютона с переходом к модифицированному методу Ньютона после k итераций
...
# Метод Ньютона с пересчётом матрицы Якоби не каждый раз, а period_given итераций
